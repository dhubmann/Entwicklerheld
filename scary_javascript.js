...
export function getQuantityOfIngredient(ingredient, numberOfCakes){
    ...
    for (var ingredientOfCake of RECIPE.ingredients.cake){
        if(ingredientOfCake.name === ingredient){
            ...
            if(ingredient === "eggs"){
                quantityOfIngredient = ingredientOfCake.quantity * numberOfCakes + numberOfCakes;
...
export function getPurchaseList(){
    ...
    for(let ingredient of ingredients){
        calculator_funcs.push(function(result) {
            let quantityOfIngredient = result[ingredient.name] ? result[ingredient.name] : 0;
    ...
    let result = {};
    for (let calculator_func of calculator_funcs) {
...
export function calculateDifficulty(){
    let arrayOfTimers = RECIPE.times.cake.concat(RECIPE.times.buttercream);
    let difficulties = [];
    for(let timer of arrayOfTimers){
        if(timer == 0){
